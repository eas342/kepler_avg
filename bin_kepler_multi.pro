pro bin_kepler_multi,zoomIn=zoomIn,targ=targ,doSecondary=doSecondary
...
;;        KIC 12557548b
;; 
;; doSecondary - find the Secondary eclipse

if n_elements(targ) EQ 0 then targ='KIC1255'
case targ of
   'KIC1255': begin
      saveFile='output/avg_kep/avg_kep.sav'
      outName='output/avg_kep/avg_bin_kep.sav'
      binsize = 0.01E
   end
   'KIC1255LC': begin
      saveFile='output/avg_kep/avg_kep_lc.sav'
      outName='output/avg_kep/avg_bin_kep_lc.sav'
      binsize = 0.01E
   end
   'K2-22': begin
      saveFile='output/avg_kep/k2_22.sav'
      outName='output/avg_kep/avg_bin_k2_22.sav'
      binsize = 0.0016E
      oreject= 2.0
   end
   'HAT-P-7': begin
      saveFile='output/avg_kep/hat_p_7.sav'
      outName='output/avg_kep/avg_bin_hat_p_7.sav'
      binsize = 0.002E
      oreject = 3.0
   end
   'TrES-2': begin
      saveFile='output/avg_kep/tres_2.sav'
...
if keyword_set(doSecondary) then begin
   saveFile = clobber_exten(saveFile) + '_sec.sav'
endif
restore,saveFile
